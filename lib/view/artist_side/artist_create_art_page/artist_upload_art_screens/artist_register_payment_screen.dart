...
import '../../../../keys.dart';
import 'artist_exhibition_card_screen.dart';
import 'package:http/http.dart' as http;

class ArtistRegisterPaymentScreen extends StatefulWidget {
  final Map<String, dynamic> response;
  const ArtistRegisterPaymentScreen({Key? key, required this.response})
      : super(key: key);

  @override
  State<ArtistRegisterPaymentScreen> createState() =>
      _ArtistRegisterPaymentScreenState();
}

class _ArtistRegisterPaymentScreenState
    extends State<ArtistRegisterPaymentScreen> {
  @override
  void initState() {
    print(widget.response);
    print("Status: ${widget.response['status']}");
    print("Message: ${widget.response['message']}");
    print("Is Paid: ${widget.response['is_paid']}");
  ...
  Future<void> registerForExhibition(BuildContext context) async {
    ...
    final response = await apiService.registerForExhibitionForFreeSeller(
    ...
    if (response != null) {
      ...
      if (response['status'] == 'true') {
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) =>
                ArtistExhibitionCardScreen(exhibitionData: response['data']),
          ),
        );
      } else {
        setState(() {
  ...
  Future<void> cancelForExhibitionSeller(BuildContext context) async {
    setState(() {
      isCancel = true;
    });
    final response = await apiService
        .cancelForExhibitionSeller(widget.response['registration_code'].toString());
  ...
  Future<void> registerForExhibitionPaid(
    ...
    final response = await apiService.registerForExhibitionForPaidSeller(
        widget.response['registration_code'].toString(),
        payment_id.toString(),
            ...
            child: Column(
              children: [
                ...
                Center(
                    child: GestureDetector(
                  onTap: () {
                    cancelForExhibitionSeller(context);
                  },
                  child: Container(
                    height: Responsive.getHeight(50),
                    width: Responsive.getWidth(335),
