...
import 'package:artist/view/user_side/profile_screen/profile_screens/order_screen/return_order_chat_screen.dart';
...
import '../../../user_side/notification/notification_screen.dart';
import '../../artist_my_art_page/single_art_screen/artist_review_art_screen.dart';
import 'artist_return_order_chat_screen.dart';

class ArtistReturnOrderAllChatScreen extends StatefulWidget {
  const ArtistReturnOrderAllChatScreen({super.key});

  @override
  State<ArtistReturnOrderAllChatScreen> createState() =>
      _ArtistReturnOrderAllChatScreenState();
}

class _ArtistReturnOrderAllChatScreenState extends State<ArtistReturnOrderAllChatScreen> {
  @override
  void initState() {
    super.initState();
    _loadUserData();
  ...
  Future<void> fetchChatData() async {
    try {
      final response = await http.post(
        Uri.parse(
            '$serverUrl/seller_getting_return_form'),
      ...
      if (response.statusCode == 200) {
        ...
        if (data["status"] == true) {
          ...
          setState(() {
            chats = List<Map<String, dynamic>>.from(data["return_enuqiry_list"]);
          });
          print(chats);
        } else {
          setState(() {
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  SizedBox(width: Responsive.getWidth(30)),
                  WantText2(
                      text: "Return Order Enquiry",
                  ...
                  onTap: () {
                    ...
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        ...
                        builder: (_) => ArtistReturnOrderChatScreen(
                          fcmToken: chat['reciver_fcm_token'],
                          art_id: chat['art_unique_id'],
                          fcm_token: chat['reciver_fcm_token'],
                          // art_type: chat['enquiry_category_name'],
                          isReturnApproved: chat['isReturnApproved'],
                          art_name: chat['title'],
                          customerUniqueID: customerUniqueID,
                          reciver_unique_id: chat['reciver_unique_id'],
                          art_enquiry_chat_id: chat['last_message'] != null
                              ? chat['last_message']['return_order_id'].toString()
                              : chat['return_order_id'].toString(),
                        ),
                      ),
                    );
                    // }
                              ...
                              children: [
                                          ...
                                          MaterialPageRoute(
                                              builder: (_) =>
                                                  ArtistSingleArtScreen(
                                                    artUniqueID:chat['art_unique_id'],
                                                  )));
                                    },
                                    child: ClipRRect(
                                      child: Image.network(
                                ...
                                Column(
                                  ...
                                  children: [
                                    ...
                                    chat['last_message'] != null ? SizedBox(
                                      width: 200,
