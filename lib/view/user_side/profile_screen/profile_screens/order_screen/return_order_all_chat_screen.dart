...
class ReturnOrderAllChatScreen extends StatefulWidget {
  const ReturnOrderAllChatScreen({super.key});

  @override
  State<ReturnOrderAllChatScreen> createState() =>
      _ReturnOrderAllChatScreenState();
}

class _ReturnOrderAllChatScreenState extends State<ReturnOrderAllChatScreen> {
  @override
  void initState() {
    super.initState();
    _loadUserData();
  ...
  Future<void> fetchChatData() async {
    try {
      final response = await http.post(
        Uri.parse(
            '$serverUrl/get_customer_all_return_enquiry'),
      ...
      if (response.statusCode == 200) {
        ...
        if (data["status"] == true) {
          ...
          setState(() {
            chats =
                List<Map<String, dynamic>>.from(data["return_enuqiry_list"]);
          });
        } else {
          setState(() {
            isLoading = false;
                      ...
                      return GestureDetector(
                        onTap: () {
                          ...
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (_) => ReturnOrderChatScreen(
                                ...
                                customerUniqueID: customerUniqueID,
                                reciver_unique_id: chat['reciver_unique_id'],
                                art_enquiry_chat_id: chat['last_message'] !=
                                        null
                                    ? chat['last_message']['return_order_id']
                                        .toString()
