...
class SinglePrivateProductDetailScreen extends StatefulWidget {
  final artUniqueId;
  const SinglePrivateProductDetailScreen(
      {super.key, required this.artUniqueId});

  @override
  State<SinglePrivateProductDetailScreen> createState() =>
      _SinglePrivateProductDetailScreenState();
}

class _SinglePrivateProductDetailScreenState
    extends State<SinglePrivateProductDetailScreen> {
  int selectedIndex = 0;
  ...
  Future<void> fetchArtDetails() async {
    try {
      var response = await ApiService().fetchSinglePrivateArtDetails(
        widget.artUniqueId,
      );
                          ...
                          child: Column(
                            ...
                            children: [
                              Container(
                                ...
                                child: artDetails.categoryArtData[index]
                                    .artImages.length ==
                                    0
                                    ? Center(
                                  child: WantText2(
  ...
  void showRegisterDialog(
      BuildContext context, String artUniqueId, String artist_unique_id) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (BuildContext context) {
    ...
    if (_formKey.currentState!.validate()) {
      if (nameController.text.isEmpty) {
        ...
        try {
          ...
          //   );
          // });
          final response = await ApiService().addPrivateSaleEnquiry(
